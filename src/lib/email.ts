...
export async function sendAnalyticsReport(
  ...
  // When a PDF is attached, keep the email body short — the PDF has the
  // full report.  Fall back to the full inline report when there's no PDF.
  const hasPdf = !!(opts.pdfBuffer && opts.pdfFilename);

  const briefHtml = `<!DOCTYPE html>
<html>
<head>
<style>
  body { font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; color: #1a1a1a; padding: 20px; max-width: 600px; margin: 0 auto; }
...
<div class="footer">Generated by Tilt Agent Orchestrator</div>
  ...
  const briefText = `${subject}\n\nYour report is attached as a PDF.\n\nGenerated by Tilt Agent Orchestrator`;

  const emailPayload: Parameters<typeof resend.emails.send>[0] = {
    from: opts.from,
    to: opts.to,
    subject,
    text: hasPdf ? briefText : opts.reportText,
